...
import com.xlm.trigger.api.IRaffleStrategyService;
    ...
    public Response<List<RaffleAwardListResponseDTO>> queryRaffleAwardList(@RequestBody RaffleAwardListRequestDTO request) {
        try {
            ...
            String[] treeIds = strategyAwardEntities.stream()
                    ...
                    .filter(ruleModeds -> ruleModeds != null && !ruleModeds.isEmpty())
                    .toArray(String[]::new);
            // 4. 查询规则配置 - 获取奖品的解锁限制，抽奖N次后解锁
            // 查询rule_tree_node的rule_key字段为rule_lock的，组成map<tree_id, rule_value>
